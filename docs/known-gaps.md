...
| A11 | **关掉保护期间建立的连接,重新 `bx up` 之后仍从物理网卡直出**(2026-09-25 真机抓包:`bx down` 那 14 秒里开的 TCP 连接,`bx up` 之后又在 en0 上跑了一分半,直到自己关掉)。macOS 已连接的 socket 不会因为路由表变了而改走 TUN;所有 VPN 都有同一个性质,但 bx 的承诺是「不泄漏」,而用户从菜单开关一次保护就会碰上。修法要么在 up 时让那些连接断掉(`pf` 按源地址 reset?),要么在菜单与 `bx up` 的输出里明说。**不是切换本身的问题。** | 2026-09-25 | `docs/acceptance-pending.md` B9 | 待定 |
