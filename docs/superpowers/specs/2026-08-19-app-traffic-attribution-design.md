...
**「unknown 占比高」是一个可用的故障信号(2026-08-20 起)** —— 报告只覆盖最近
60 秒(`appattr.ReportWindow`),归因由一条 250ms 的后台 resolver 在连接**还开着的
时候**做完并存进记录。两件事一起,`unknown` 不再随开窗时长单调增长。
此前这一段写的是反话(「别据此读成故障」),那是在两件事都还没做时的正确忠告;
今天它已经不成立,原委见「已知缺口」里那一条。
...
**「第二句常驻小字会变墙纸」这个反对意见被证伪过一次,记在这里免得重提**:项目
所有者否掉「Direct rules: N unreachable」的**承重理由是它需要定期主动探测隧道外面**
(泄漏检测工具新增不受保护的出站 + 假阳性极便宜),常驻只是附带;而这个窗口上
**已经有一句同样恒真、同样不可行动的小字并且已被接受**(近似值那条)。两句的共同点
是**对眼前这份数据的准确性声明**,不是对系统状态的告警 —— 按这个分类,第二句与
第一句同类。「不可行动」那半仍然成立,处置见下面那条待办。

**布局上两条承重的细节**(都不是讲究):
- **凡是会截断的格子,必须同时给出看全的办法。** 规则原文是最后一列、变长、
  `byTruncatingTail`,而这个窗口**不横向滚动**(clip 的宽度锚死在 scroll 上)——
  少了出路,一条被截断的规则就**永久不可见**,而上一版那句散文是整行读得到的,
  那会是净退化。两条出路:那一格的 `toolTip`(不用改布局)与窗口的 `.resizable`
  (看得见、拖一下就有)。
- **挤压顺序必须是确定的**:规则原文(250)< 应用名(500)< 数字列(750)。
  第一版应用名与五个数字格同为默认的 750,规则列压到零之后「谁再让位」由 Auto
  Layout 在同优先级里任选,而 `Microsoft Teams (work or school)` 这类名字长度有
  真实上界但不小。守卫钉的是**方向**(把两个数字对调必须转红),以及**哪一格用
  哪一档**(阶梯定义得再对,格子照旧走 `hint(...)` 就一档都没用上)。
...
- ~~**报告是「自打开窗口以来的累计」,不是「此刻」;`unknown` 因此会结构性膨胀。**~~
  **已修(2026-08-20),而且这条缺口原本把因果写反了 —— 记在这里,因为写反的
  方式很典型。** 原文说:只要窗口开着,`records` 与两张字节表永不按时间裁剪,而
  `owners` 是 Snapshot 当下的「此刻还开着的 socket」快照,所以几分钟前关掉的连接
  再也查不回主人;并断言「短连接在 worker 解析前就关闭确实也会落进 unknown,但
  那是同一现象的一个瞬时特例,**不是它的主因**」。
  **真机证据把这句话翻了过来**:项目所有者的生产 Mac 上,`Unknown app` 是最大的
  一行(16 条连接),**而且是唯一有真实速率的一行**(959 B/s 上 / 1.8 KB/s 下)。
  速率是决定性的 —— 如果 unknown 主要是陈旧记录堆出来的,累计值就不会涨、速率
  应该是 0;速率不为零就说明**新的 unknown 记录在源源不断地进来**。
  **主因是「归因发生在读取时」**:`OwnersByPort()` 全仓只在 `Snapshot()` 里被调
  一次(菜单 5 秒一拍),于是**任何活不过一次刷新的连接,在被归因之前 socket 就
  没了** —— 内核里查不到那个端口,结构性地落进 unknown。「不按时间裁剪」让这些
  记录**留得太久**(只涨不落),但它是放大器,不是产生者。
  **两件事一起修**:① `ConnRecord` 带上时间戳,报告只覆盖最近
  `appattr.ReportWindow`(60 秒),字节账跟着裁(**只裁一半等于没裁**);
  ② 订阅期间起一条 250ms 的后台 resolver,在连接**还开着的时候**把归因解析出来
  并**存进记录**,于是连接关掉之后归因仍然在。至此「占比高 ⇒ 有问题」这条判据
  重新成立,「只看开窗头十几秒」那个替代办法也不再需要。
  **一个必须一起做对的细节**:窗口不能只按时间裁 —— **还开着的连接不裁**,
  否则一条开了两小时还在灌流的会议媒体流会在开窗 60 秒后消失,那正是
  「订阅之前建立的连接看不见」那个真机 bug 换一种方式复发。
...
- **待办:给种子进来的行打标记,让那句小字只在它真的适用时出现(2026-08-20 判定,
  不现在做)。** 那句小字今天是**恒常**的:一份完全没有种子行的报告也照样显示它,
  而它**不可行动**(用户读完能做的只有「下次开会前先开窗口」)。更好的形状是
  `AppRow` 带一个 bool(这一行含订阅之前就建好的连接),小字变成**那一行旁边的
  一个注记** —— 不是窗口底部的第三句话:底部那两句是「整份数据的准确性声明」,
  而这一条是**某几行**的性质,摆在底部就等于说给所有行听。
  **顺序上它排在「真机验八列布局」之后** —— 布局本身一次没上过屏幕,在那之前给
  行内再加一个注记是在没验过的地基上加东西。
