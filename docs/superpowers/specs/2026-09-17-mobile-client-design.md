...
> **L1 当天就被撤回了(2026-09-17,项目所有者)。** 理由是「对用户要求太高」,
> 而且是对的:导出一份文件之后要想办法传到手机、导入客户端,**以后每次改一条规则
> 都得重来一遍**。它不是一次性设置,是一件长期的家务;而一个需要用户手动保持同步的
> 方案,**比两边策略不一样更糟** —— 因为你会以为它们一样。
>
> 真要解决同步,形状是**订阅 URL**(客户端定期去拉)。但今天 `bx server` 那一侧
> 没有任何托管配置的能力,做成订阅要回答三个新问题:真相源在 Mac 的
> `/etc/bx/config.yaml` 而 VPS 不知道 ⇒ 要一条新的推送与认证通道;托管一份带
> 你全部规则的配置在 VPS 上是**新的暴露面**(里面有内网域名与公司 DNS 地址)。
> **而这三个问题的答案在有自己的 app 时会完全不同** —— 现在做它,是为一个可能
> 不会存在的中间态付全价。
>
> 所以:**不做 L1。** 下面那句"第一个组件"的话保留原文,是因为它当时的推理没错,
> 错的是我只算了工程成本、没算用户成本。
