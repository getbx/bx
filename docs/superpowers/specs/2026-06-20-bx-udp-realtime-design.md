...
- 初版不把所有 UDP 默认放开。
- 已验证 brook socks5 路径支持 UDP;bx 用自有 SOCKS5 UDP ASSOCIATE dialer 承接。
...
`bx up` 即完整启动透明代理。用户不需要理解 TCP/UDP;非 DNS UDP 默认通过 bx 隧道中继。
...
- `on`:默认模式。非 DNS UDP 通过 bx 隧道中继,避免走本地真实网络路径。
- `off`:高级阻断模式。显式阻断非 DNS UDP。
...
```yaml
udp:
  mode: proxy # proxy | block | direct-realtime
```

未写 `udp.mode` 时默认 `proxy`。`bx realtime on` 只是把高级策略恢复到默认 `proxy`。`direct-realtime` 仅保留为兼容/应急配置,不作为推荐用户路径。
...
- `sudo bx realtime on`:恢复默认 UDP relay 策略,运行中自动重启生效。
- `sudo bx realtime off`:高级:显式回到 `block`。
...
```text
UDP     mode proxy  阻断 0  non-DNS UDP relayed through bx tunnel
```

`bx doctor --json` 默认报告 UDP relay ok。仅当用户显式切到 block 时给出 hint:

```json
{
  "name": "udp_policy",
  "status": "warn",
  "detail": "non-DNS UDP blocked",
  "hint": "Google Meet/WebRTC may stutter; use sudo bx realtime on"
}
```
...
2. 加 config `udp.mode=proxy` 默认和测试。
...
- Google Meet 或 WebRTC test 页面在默认 `bx up` 与 `realtime off` 下对比;
- `bx down` 后 DNS/路由仍恢复;
- 在 compass Wi-Fi 这种已有上游分流网络中,对比默认 `bx up` 与 `bx down` 的会议上行质量。
