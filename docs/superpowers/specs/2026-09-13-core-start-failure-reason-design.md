...
**日期**:2026-09-13
**状态**:设计,待实施
**起因**:2026-09-12 真机事故。所有者的 VPS(195.133.192.92)连 ssh 与 ping 都不通,而 bx 对着
...
Core 从第一秒就知道:`dial tcp 195.133.192.92:443: i/o timeout`。它被逐层剥掉:
...
**以及用户配了第二台**(他配了 195.133.192.92 与 166.1.190.123,bx 一个字没提)。
...
```
bx 起不来:连不上服务器 195.133.192.92:443 —— 那台机器上的这个端口没有应答。
  · 它可能挂了、或者换了 IP。确认:nc -z 195.133.192.92 443
  · 你还配了另一台:166.1.190.123 —— sudo bx server use <名字>
```

`tunnel_handshake_failed`(措辞必须相反,否则等于把人派去修一台好机器):

```
bx 起不来:服务器 195.133.192.92:443 在应答,但隧道 20 秒内没能建起来。
