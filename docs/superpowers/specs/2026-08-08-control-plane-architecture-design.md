...
做到这一步,以下手写补偿**部分**可以删除——它们会变成同一个循环的普通输入:

> **2026-08-09 更正(测绘实证,详见 `2026-08-09-stage3-reconciler-design.md`)**:
> 下面这份清单里**只有两条真的可删**。
> · **升级欠条不可删**:它存在的原因是升级把「停下 Core 换二进制」表达成了
>   `desired=off`,于是**任何忠实的调谐器都会把它收敛到 off,而那正是 bug 本身**。
>   删它需要的是数据模型改动(与 desired 正交的「维护挂起」),不是循环。
> · **孤儿屏障清理只能删一半**:Guardian 侧可以,CLI 侧那份必须留——逃生口的定义
>   就是「Guardian 已死时可用」,而那时没有循环在跑。
> · **进程扫描反而更承重**:它不是修复陈旧状态,是防止两个 Core 同时跑的**准入控制**。
>   循环若按时钟起 Core,它每一轮都要跑。
> 另外:**屏障的安装绝不该由时钟驱动**——一次瞬时的网关探测失败会让它降级成
> block-only(无 server bypass)的整机黑洞,而循环每拍都在重掷这个骰子。
