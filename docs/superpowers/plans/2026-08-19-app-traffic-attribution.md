...
- [ ] **真机验收清单**(要人在屏幕前):
  ...
  4. **(2026-08-20 已作废,留原文备查)** 下面这条限定连同它背后的因果都是错的:
     真机上 unknown 是最大的一行**且带着真实速率**,证明新记录在源源不断进来;
     主因是**归因发生在读取时**(`OwnersByPort` 只在 `Snapshot` 里调),不是
     「不按时间裁剪」。现已改成 60 秒滚动窗口 + 250ms 后台 resolver,`unknown`
     占比在**任何时刻**都该低。原文:
     确认**开窗后 10 秒内** `unknown` 行占比不高。**只在这个窗口里量** ——
