...
**先看这一条,否则那两个数字都是假的**:日志与 `bx status` 都要读**证据质量**。
三项探测全失败时,判断恰好等于一台健康机器的判断(Unknown 一律「什么都不做」),
于是「0 次提议」既可能是「一切正常」,也可能是「什么都没看见」。日志行的
`unobservable=` 与 `bx status` 的「N 项未观测到」就是分界线 —— 有它就说明这一轮
是真的看过了。

1. **`guardian_reconcile_would` 出现了几次、内容是什么?**
   - 位置是 **`/var/log/bx-guard.err.log`**(`log.Printf` 走 stderr;`bx-guard.log`
     里没有)。
   - 只数 `actions=` **不是** `none` 的行。这条日志在**差异发生变化**时打,一次瞬时事件
     会留下两行(变糟一行、恢复一行),按出现次数数会翻倍。
   - 一台健康机器上理想是 **0 次**。若持续提议某个动作,说明判据或观测有问题 ——
     **而这正是本期存在的理由:在授权之前把它测出来。**
   - **周期会退避到 10 分钟上限**(约 5 轮无变化后达到,稳态约 150 轮/天)。短于 10 分钟
     的分歧会被整轮错过 —— 这个数字是个**下界**,不是全貌。
2. **`looksLikeCore` 的误报率。** 它按 argv 判定、刻意不匹配可执行路径
   (`2026-08-09-down-must-not-lie-design.md:107-111` 标记为**从未测量**)。
   循环现在每轮只读地扫一次,答案在 `guardian_core_scan reason=observe` 与
   `bx status` 的「扫到 N 个 Core 进程」里:保护开着时 N 应恒为 1,关着时恒为 0。
   **`core_scan=2`(或关着时的 1)就是一次误报**;「未测成」与「测到 0 个」是两回事,
   报告里分得开。
   (准入路径那条普查是 `reason=lifecycle`,别混着数 —— 分标签正是为了这个。)

**soak 必须留一段时间把保护关掉。** `decide` 在 `desired=on` 时只有一条规则
(`CoreSocket`),而③b 打算最先授权的两项(`restore_dns`、`clear_orphan_barrier`)
**只存在于 `desired=off` 分支**。一台全程开着保护的机器跑一整天,对那两项零证据。
Guardian 在 `KeepAlive` 下继续跑,循环照常滴答。

**顺手确认循环是活的**:`bx status` 的 `Loop` 一行。Guardian 起来一分钟后还说
「尚未完成第一轮观测」,就是循环没在跑;「最近观测」超过 ~20 分钟(退避上限的两倍)
会被标成陈旧,那时去日志里找 `guardian_reconcile_loop_panicked`。
