...
type liveConn struct {
	...
	source string
	rule   string
	// dest 是这条流连的是谁(域名或裸 IP,见 appattr.ConnRecord.Dest)。
	//
	// **一条流一个条目之后,它不再是近似**:同一个源端口上并存的多条流各有各的
	// 条目、各留各的 dest,播种时一条流发一条记录,两个目的地都报得出来。端口
	// **复用**(旧流已结束、新流接手同一个号)则只报接手的那一个 —— 那才是对的:
	// 这张表答的是「此刻在连什么」,把历史目的地一起报出来会把它悄悄变成一份
	// 历史记录,隐私性质完全不同。
	dest string
}
