...
// **能力键缺席**(旧 Guardian:Status 结构体压根没有 capabilities 字段,
// 这份应答忠实模拟那种情况)必须被 StatusCapabilities 认出来——declared 要
// 是 false。这是 bx status --watch 能力门控要拒绝的第一种「没有」。
func TestClientStatusCapabilitiesDetectsAbsentKey(t *testing.T) {
	client := &Client{HTTPClient: &http.Client{Transport: recoveryRoundTripperFunc(func(*http.Request) (*http.Response, error) {
		return &http.Response{
			StatusCode: http.StatusOK,
			Header:     make(http.Header),
			Body:       io.NopCloser(strings.NewReader(`{"schema_version":1,"desired":"on","protection_state":"protected","guardian_version":"0.9.0-old"}`)),
		}, nil
	})}}
	status, declared, err := client.StatusCapabilities(context.Background())
	if err != nil {
		t.Fatalf("StatusCapabilities() error = %v, want nil", err)
	}
	if declared {
		t.Errorf("declared = true,want false(响应里根本没有 capabilities 键)")
	}
	if status.GuardianVersion != "0.9.0-old" {
		t.Errorf("GuardianVersion = %q,want 0.9.0-old(其余字段仍要正常解出,供报错信息使用)", status.GuardianVersion)
	}
}

// **能力键出现但为空数组**,与上一条相对:declared 必须是 true,即使
// Capabilities 本身 len()==0——这正是"键缺席"与"声明了、但为空"这两种
// "没有"在 Go 侧唯一能区分开的地方,若只看 len(Capabilities) 会把它俩混为一谈。
func TestClientStatusCapabilitiesDetectsPresentButEmpty(t *testing.T) {
	client := &Client{HTTPClient: &http.Client{Transport: recoveryRoundTripperFunc(func(*http.Request) (*http.Response, error) {
		return &http.Response{
			StatusCode: http.StatusOK,
			Header:     make(http.Header),
			Body:       io.NopCloser(strings.NewReader(`{"schema_version":1,"desired":"on","protection_state":"protected","capabilities":[]}`)),
		}, nil
	})}}
	status, declared, err := client.StatusCapabilities(context.Background())
	if err != nil {
	...
	if !declared {
		t.Errorf("declared = false,want true(capabilities 键出现过,只是声明的列表是空的)")
	}
	if len(status.Capabilities) != 0 {
		t.Errorf("Capabilities = %v,want 空", status.Capabilities)
	}
}

// 能力键出现且真的含 status_watch——declared=true 且
// slices.Contains(status.Capabilities, CapabilityStatusWatch) 应为 true,
// 这是让 bx status --watch 放行的唯一形状。
func TestClientStatusCapabilitiesDetectsStatusWatchPresent(t *testing.T) {
	client := &Client{HTTPClient: &http.Client{Transport: recoveryRoundTripperFunc(func(*http.Request) (*http.Response, error) {
		return &http.Response{
			StatusCode: http.StatusOK,
			Header:     make(http.Header),
			Body: io.NopCloser(strings.NewReader(
				`{"schema_version":1,"desired":"on","protection_state":"protected","capabilities":["status_watch","rules"]}`,
			)),
		}, nil
	})}}
	status, declared, err := client.StatusCapabilities(context.Background())
	...
	if !found {
		t.Errorf("Capabilities = %v,应包含 %q", status.Capabilities, CapabilityStatusWatch)
	}
}
