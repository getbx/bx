...
//
// 就地截断时,改写一份已有欠条中途崩溃会留下半截文件 —— 这个文件存在的意义
// 正好被反过来吃掉。rename 覆盖会换掉目录项指向的 inode,这里就用 inode 变化
// 钉死「不是就地写」。
func TestSaveUpgradeIntentReplacesFileAtomically(t *testing.T) {
	path := filepath.Join(t.TempDir(), "upgrade-intent.json")
	store := OpenStore(Paths{UpgradeIntent: path})
	...
	if pending, present := store.LoadUpgradeIntent(); !present || !pending {
...
func TestLoadUpgradeIntentTreatsCorruptFileAsDebt(t *testing.T) {
	...
	pending, present := OpenStore(Paths{UpgradeIntent: path}).LoadUpgradeIntent()
	if !present || !pending {
...
// 销账幂等:文件本来就不在不算失败。
func TestClearUpgradeIntentIsIdempotent(t *testing.T) {
	path := filepath.Join(t.TempDir(), "upgrade-intent.json")
	store := OpenStore(Paths{UpgradeIntent: path})
