...
// 本文件只做**判断**,不做任何事。**这句话的范围是本文件,不是调谐环** ——
// 循环今天有执行权,授权面是 executableReconcileActions(reconcile_execute.go),
// 那里也是唯一授予执行权的地方。
//
// 这里抽出的是那半个「判断」:把用户想要什么(Desired)与系统现在看起来是什么样
// (observe.ObservedState)一对,产出一组**命名的意图**。判断与执行分家的整个
// 意义就在于此 —— 提议一项动作与被允许做它是两件事,中间隔着那份白名单。
//
