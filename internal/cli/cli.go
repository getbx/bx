...
func New() *cli.App {
	return &cli.App{
		...
		Commands: []*cli.Command{
			guardianCommand(),
			{Name: "setup", Category: "First run", Usage: "first-time setup: write the config, install the service, check connectivity (does not start it)", ArgsUsage: "bx://...", Flags: setupFlags(), Action: setupAction},
			{Name: "probe", Category: "First run", Usage: "check that a bx:// link connects (writes no config, touches no routes)", ArgsUsage: "bx://...", Flags: probeFlags(), Action: probeAction},
			{Name: "server", Category: "Server side", Usage: "manage a bx server", Subcommands: serverCommands()},
			{Name: "invite", Category: "Server side", Usage: "generate a bx invite for an ordinary user", ArgsUsage: "[name]", Flags: inviteFlags(), Action: inviteAction},
			{Name: "user", Category: "Server side", Usage: "manage bx users", Subcommands: userCommands()},
			{Name: "preset", Category: "Routing rules", Usage: "apply the built-in app-reachability rules", Subcommands: presetCommands()},
			{Name: "doctor", Category: "Diagnose", Usage: "diagnose the client's configuration and runtime state", Flags: doctorFlags(), Action: doctorAction},
			{
				Name:      "explain",
				Category:  "Diagnose",
			...
			{Name: "inspect", Category: "For agents", Usage: "print a diagnostic bundle an agent can read", Flags: inspectFlags(), Action: inspectAction},
			{
				Name:     "leak-check",
				Category: "Diagnose",
				Usage:    "the same check as leakcheck, but non-interactive JSON — for scripts and MCP, opens no page",
				Description: "Non-interactive, machine-readable leak check: this machine's leak surface, coexisting VPNs " +
					"(Tailscale / ZeroTier / WARP) and an optional active exit probe.\n\n" +
					"If you are a human, type `bx leakcheck` (no hyphen) instead: it opens a local page and lines the " +
					"browser half up against the machine half, and only both halves together can decide whether you are leaking.",
				Flags:  leakCheckFlags(),
				Action: leakCheckAction,
			},
			{
				Name:     "leakcheck",
				Category: "Diagnose",
				Usage:    "leak check (this is the one to type): opens a browser page and checks both halves",
				Description: "Opens a one-off local page and lines the browser half (WebRTC, public exit IP) up against " +
					"the machine half (routes, DNS). Only both halves together can decide whether you are leaking — " +
					"either one alone can look fine while the other does not.\n\n" +
					"It works with protection off and with another VPN running, needs no root and reads no config.",
				Flags:  leakcheckFlags(),
				Action: leakcheckAction,
			},
			{Name: "observe", Category: "Diagnose", Usage: "watch runtime state change over a short window (read-only)", Flags: observeFlags(), Action: observeAction},
			{Name: "apps", Category: "Diagnose", Usage: "traffic by app: which app goes through the tunnel, which goes direct, which is blocked (read-only, samples a short window)", Flags: appsFlags(), Action: appsAction},
			{Name: "capabilities", Category: "For agents", Usage: "print a machine-readable capability list", Action: capabilitiesAction},
			...
			{Name: "reconnect", Category: "Diagnose", Usage: "troubleshooting: trigger a safe recovery by hand (ordinary network changes recover on their own)", Flags: reconnectFlags(), Action: reconnectAction},
			...
			{Name: "direct", Category: "Routing rules", Usage: "manage the direct allowlist (in global mode only allowlisted domains go direct, everything else is tunnelled)", Subcommands: directCommands()},
			{Name: "proxy", Category: "Routing rules", Usage: "manage the domains forced through the tunnel", Subcommands: proxyCommands()},
			{Name: "dns", Category: "Routing rules", Usage: "manage the macOS system DNS takeover", Subcommands: dnsCommands()},
			{Name: "realtime", Category: "Routing rules", Usage: "show the realtime UDP policy", Subcommands: realtimeCommands()},
			...
			{Name: "tray", Category: "Windows only", Usage: "start the system tray (Windows; click the icon to connect, disconnect, configure, or see status)", Action: trayAction},
			{Name: "autostart", Category: "Windows only", Usage: "start-at-boot switch (Windows; on|off|status)", ArgsUsage: "on|off|status", Action: autostartAction},
			...
			{Name: "mcp", Category: "For agents", Usage: "start the agent control-plane MCP server (stdio)", Hidden: false, Flags: mcpFlags(), Action: mcpAction, Subcommands: []*cli.Command{
			...
			{Name: "uninstall", Category: "First run", Usage: "uninstall the client service", Action: uninstallAction},
			{Name: "egress", Category: "Routing rules", Usage: "named egress: hand named prefixes to a tunnel you already have (the fallback for when a mesh will not work)", Subcommands: egressCommands()},
		},
	}
}
