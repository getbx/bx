...
func upDoneMessage() string {
	return "✅ bx started. (start-at-boot is unchanged; adjust it from the tray's \"Start at boot\" item or with bx autostart)"
}

// downDoneMessage 同理:down 不再顺带取消开机自启。
func downDoneMessage() string {
	return "✅ bx stopped. (start-at-boot is unchanged)"
}
