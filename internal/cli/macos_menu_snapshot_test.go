...
func checkTree(t *testing.T, path string) {
	...
	// **一句被截断的话与一个越界的按钮是同一类缺陷**:确定性判定,不是审美问题。
	//
	// 2026-09-18 量到的原形:规则窗口 420pt 宽,而三条预设副标题**全部**在句子
	// 中间断掉(需要 ~347pt,实得 218~269pt)。那句话正是 2026-09-14 定下来用于
	// 回答「开了会怎样」的东西 —— 那个设计在默认宽度下 100% 失效,而没有任何
	// 东西会报错:AppKit 画一个省略号,截图看起来很正常。
	//
	// dump 只给**不能换行**的标签打这个标记(会换行的字段超出边框只说明它折了行,
	// 那是对的),所以这里可以无条件红。
	for _, line := range lines[2:] {
		if strings.Contains(line, " truncated needs=") {
			t.Errorf("这一行字被截断了(默认窗口宽度装不下它):\n  %s\n"+
				"用户读到的是半句话,而 AppKit 只画一个省略号、不报错。"+
				"要么把窗口加宽,要么把文案缩短 —— 别让它继续悄悄断着。",
