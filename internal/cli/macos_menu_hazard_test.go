...
func TestOpenPlatformListMatchesPolicy(t *testing.T) {
	...
	// 从 policy.go 的**源码文本**里抽出 riskyDirect 的字面量清单(与本文件读
	...
	for _, host := range goList {
		if !swiftSet[host] {
			t.Errorf("policy.riskyDirect 里有 %q,而 Swift 的 openSubdomainPlatforms 里没有 —— 两份漂开了", host)
		}
	}
	for _, host := range swift {
		if !goSet[host] {
			t.Errorf("Swift 的 openSubdomainPlatforms 里有 %q,而 policy.riskyDirect 里没有 —— 两份漂开了", host)
		}
	}
}

// readOpenSubdomainPlatformsFromPolicySource 从 internal/policy/policy.go 的
// 源码文本里抽出 riskyDirect 的字面量清单。读不出来必须 t.Fatal —— 正则没
...
func readOpenSubdomainPlatformsFromPolicySource(t *testing.T) []string {
	...
	block := regexp.MustCompile(`(?s)var riskyDirect = route\.NewDomainSet\(\[\]string\{(.*?)\}\)`).FindStringSubmatch(string(src))
	if block == nil {
		t.Fatal("读不出 policy.go 里的 riskyDirect —— 守卫已失效,先修守卫")
	}
	var out []string
	for _, raw := range regexp.MustCompile(`"([^"]+)"`).FindAllStringSubmatch(block[1], -1) {
	...
	if len(out) == 0 {
		t.Fatal("policy.go 里的 riskyDirect 清单是空的 —— 守卫读错了地方")
	}
	return out
}
