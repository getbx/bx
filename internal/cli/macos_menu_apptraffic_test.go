...
// 于是「某某在某个块里」这类判据会被一段它其实管不着的源码满足)。两条今天
// 都不活跃,**但理由不同,别把它们说成同一句话**:① 那种形状在被扫的文件里
// **确实存在**(GuardianClient.swift:344),只是不在任何做括号扫描的判据经过
// 的位置;② 三个被扫的文件(main.swift / AppTrafficWindow.swift /
// GuardianClient.swift)里一处都没有。写在这里是因为下一个人加一行就可能踩到,
// 而**一句只说了一半的局限声明比不说更糟:他会以为这已经被想过了**:
//
//	① **不认原始字符串** `#"…"#` / `#"""…"""#`。碰上它会退化成「按第一个 `"`
//	   起算」,于是 `#"a"# + "("` 这种写法里的括号可能漏抹。
//	   **这种形状此刻就有一处**:GuardianClient.swift:344 的
//	   `Data(#"{"reason":"manual"}"#.utf8)`;它不在任何做括号扫描的判据经过的
//	   位置,所以今天无害 —— 真要扫到它所在的块,这条局限就会活过来。
//
