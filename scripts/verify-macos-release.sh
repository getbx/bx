...
grep -q "Restart Protection" "$RELEASE_DIR/README.txt" || fail "README missing upgrade restart note"
...
grep -q "sudo bx down && sudo bx up" "$RELEASE_DIR/install.sh" || fail "install.sh missing upgrade restart command"
