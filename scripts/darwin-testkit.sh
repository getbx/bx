...
if [[ "$EXECUTE" != "1" ]]; then
  ...
  echo "To execute: sudo BX_BROOK_LINK='brook://...' $0 --execute --server-bypass ${SERVER_BYPASS[0]}"
  fix_log_permissions
  exit 0
fi
...
set +e
BX_DEBUG=1 "$BX" run -c "$CONFIG" --brook "$BROOK" --test-timeout "${DURATION}s" >"$LOG_DIR/bx-run.log" 2>&1
RUN_STATUS=$?
set -e
