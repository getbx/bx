...
- **Guardian 无响应,或响应了却关不掉**(例如断网期间重启后恢复事务被永久阻断):自动改走强制停止,依次做四件事——请求正在运行的 core 经自己的控制面退出(由它的 defer 还原路由与 DNS)、停止 Guardian 服务、记录"已关闭"(不再开机自启)、删除屏障装下的阻断路由。任一步失败都不会中断后面的步骤,并会把失败原因和下一步一起打印出来。
