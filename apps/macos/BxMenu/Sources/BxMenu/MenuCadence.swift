...
struct RefreshGate {
    ...
    /// **`userInitiated: true` 该用在「结果必须尽快出现,而且这个诱因不重复」
    /// 的场合**:用户刚做完的一个动作(开/关/setup)是其中一种;`StatusWatch`
    /// 长轮询送来的一次真实代际号推进是另一种(main.swift 的 `runWatchLoop`)——
    /// 它和用户动作同属「离散、一次性事件」而非「隔几秒自己会再来一拍」的定时器,
    /// 若不补跑,`watchGeneration` 已经前进到新值,循环下一轮等的就是它,这一次
    /// 变化不会被再广播一遍,漏掉就是永久漏掉,不像定时器丢一拍下一拍自然补齐。
    ...
    /// 或 watch 收到一次真实代际号推进后发起的刷新,若正好被丢掉,菜单会把那个
    /// 结果拖到下一拍(关闭档下最长 30 秒,或干脆等不到——watch 事件不重复)。
    /// 补跑是**一次性的**,且只补 `userInitiated` 那一类:定时器那一拍被丢掉就
    /// 让它丢,下一个边沿自然会再来 —— 否则就成了首尾相接的满占空比(见 begin)。
