...
{attempts, failures} 表;内建列表也计数(少了它无从区分「全网在失败」与「只有我这条
...
`route.Reason` 里,两件事都在,只是从来没被放在一起。
...
- **字节账是 map + 一把全局 mutex,不是无锁定长表**(`apptraffic.go` 的
  ...
  连接」一条。
- **`unknown` 在它所属的组里单独成行**,不摊进已知应用、也不丢弃。**但别把
  「unknown 占比高」当成故障信号 —— 在正常机器上它随窗口打开时长单调增长,
  见下一条。**
- **报告是「自打开窗口以来的累计」,不是「此刻」——`unknown` 会结构性膨胀,
  那是稳态不是故障。** 只要窗口开着,菜单每 5 秒续期 ⇒ 订阅永不过期 ⇒
  `records` 与两张字节表**永不按时间裁剪**(唯一边界是 4096 条环形缓冲);
  而 `owners` 是 Snapshot 当下现问内核得到的「**此刻还开着的 socket**」快照。
  于是几分钟前就关掉的连接,端口再也查不到主人,**全部落进 unknown 行**。
  根因是「记录不按时间裁剪 + owners 是即时快照」这两件事的组合,**不是
  worker 太慢**(照那个方向去查会一路查错)。要判「此刻」,只看开窗头十几秒;
  **已知后续项:给 `ConnRecord` 加时间戳、按时间窗裁剪**(那才是让「占比高
  ⇒ 有问题」重新成立的前提)。
...
- **已知缺口:种子把一个 socket 上并存的 N 条流压成一条,而新记录不会。**
  `live` 按 `PortKey` 记、同键最后写入者胜,种子每键只发一条。于是一个会议
  socket 同时打 STUN(可能直连)+ TURN(可能走隧道)时:**订阅前**建立的只出现
  在**一个**组里、连接数恒为 1;**订阅后**建立的正确地出现在**两个**组里、连接
  数为 N。**同一个事实,按窗口打开时机给出不同答案** —— 而这正是「腾讯会议为
  什么绕一圈」要回答的问题。刻意不修:相对修复前(**完全看不见**)这仍是巨大
  改善,用户的用例答得出来、只是少一个组;真修要把键重新设计成能分辨同一 socket
...
- **图标**(收益最大):`AppRow.ExecPath` 是一次**刻意的信息面扩大** —— 此前离开
  ...
  **路径为空不画占位**:一格空白的占位图不是「没有图标」,是「这个应用的图标长
  这样」,那是另一句话。
- **速率**:相邻两次快照做差(窗口 5 秒一拍,前后两份快照就是一对现成的观测,
  不改协议、Core 侧不多记一份状态)。**第一帧不许编造速率** —— 显示 0 是在说
  「此刻没有流量」,拿累计值除以一个猜来的时长更糟;那一格是一个破折号。同一条
  纪律还管着另外三种「说不好」,**一律不给速率而不是给 0**:上一帧没有这个键
  (应用刚出现,累计里可能含着种子)、累计值往回走(环形缓冲丢最旧的记录时真的
  会发生)、两帧同一瞬间(除以零)。与 `observe.Tristate`、`leakcheck.NotChecked`
  同一条。键是 **(组, 应用名)**:同一个应用同时在两组里是常态,只按名字做键会算出
  一个谁也不是的速率。**累计值一个都没删** —— 速率答「现在多快」,累计答「一共
  多少」。判据全在纯模型 `AppTrafficRateTracker` 里(窗口那半 CI 里编不了)。
...
- **那句缺口小字**(`appTrafficPreexistingNote`):「Connections already open when
  this window opened may appear in only one section.」—— 上面「种子把并存的流压成
  一条」那个缺口此前只有三份记档和一条测试,**唯独用户看不到**,而它恰好落在这个
  窗口最初的用例上。措辞**只陈述观测到的现象、不断言原因**(与「Protection may be
  off.」同一条)。
...
**待办(排在真机验八列布局之后)**:给 `AppRow` 加一个「这一行含订阅前建好的连接」
的标记,把那句话变成**那一行旁边的注记**,而不是窗口底部的第三句 —— 底部两句是
整份数据的性质,行内那条是某几行的性质。
...
确认 BLOCKED 组出现内容;④ **开窗后 10 秒内**的 `unknown` 占比不高(**只在这个
窗口里成立** —— 之后 unknown 随窗口打开时长单调增长是预期行为,见上文「报告是
累计不是此刻」;拿一个开了十分钟的窗口去量这条,必然误报,而且指向的是错的根因);⑤ 每 5 秒重建视图树会不会把滚动位置拉回顶部(已知,未修);
