...
- **字节数是近似值,界面上必须说出来。** 按源端口记(两张定长表,无锁 O(1)),
  端口复用时旧账可能算到新连接头上。窗口底部固定一行
  `Byte counts are approximate (ports get reused).`,由守卫钉住。
- **`unknown` 在它所属的组里单独成行**,不摊进已知应用、也不丢弃。unknown 占比高
  本身就是「这份数据现在不可信」的信号。
...
确认 BLOCKED 组出现内容;④ `unknown` 占比不高(高说明 worker 太慢,那是要修的信号
不是要藏的数字);⑤ 每 5 秒重建视图树会不会把滚动位置拉回顶部(已知,未修);
