...
- **Windows**:**第 1/2 步 + 第 3 步的前两小步已做**——交叉编译通过(`embedded_other.go` brook 兜底、`paths_windows.go`)+ CI 三平台矩阵(ubuntu/macos/windows runner 跑单测,全绿)。真机环境已就绪(SSH 提权可达,`bx.exe` 已执行)。**第 3 步进度**:① **OpenTUN 已实现**——wintun `CreateTUN`→`wgbridge`(照抄 darwin,去掉 utun 命名限制,`bx0` 直用;运行时需签名 `wintun.dll` 同目录);② **DirectDialer 已实现**——`GetBestInterfaceEx` 探物理默认网卡 index → `IP_UNICAST_IF`(v4)/`IPV6_UNICAST_IF`(v6)防环,复用共享 `shouldBindToDevice`「仅公网目的地才绑」。**头号坑 IPv4 字节序**(`htonl(index)`,MSDN 怪癖)抽到 `unicastif.go` 的 `unicastIfV4Value`(纯逻辑,`bits.ReverseBytes32`)已 TDD 单测覆盖(Linux 本地可跑,不必等 windows runner)。这两块 windows-only syscall 交叉编译 amd64/arm64 + vet 过,**真机待验**。**待做**:③ `Hijack` 路由表劫持 + WFP 封 :53 防 Windows smart-multihomed DNS 泄漏 + IPv6 fail-closed(现返回未实现错误,`bx run` 在 OpenTUN 后于此止步、defer 干净还原);④ Windows Service(`bx up/down`)。**施工图见 `docs/superpowers/specs/2026-07-08-windows-tun-design.md`**(调研了 sing-box/WireGuard 的成熟做法)。
