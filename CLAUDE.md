...
- **字节账是 map + 一把全局 mutex,不是无锁定长表**(`apptraffic.go` 的
  ...
  存在时无分配),但它是**全局**锁,争用的数字只有真机能给。**未订阅时热路径连锁
  都不碰**(第一句 `t.active.Load()` 就返回)—— 「没人看时开销精确为零」那句话
  落在这里,不落在字节表的数据结构上。
...
开销为零」是这个设计的隐私前提,不能靠对方守规矩)。**全内存、不落盘、不进日志。**
未订阅时热路径的全部代价是**一次 atomic 读**,由一条测试钉住「注入的 appSource
一次都没被调用」——**不用基准测试,基准不会让 CI 转红**。
